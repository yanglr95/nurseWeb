...
(function(jQuery) {
  jQuery.fn.tagBox = function(options) {
    var defaults = {
      ...
      removeTagText: "<img src=\"../../Images/Close-icon.png\" style=\"width:2em;height:2em\"/>",
      maxTags:-1,
...
//  Hide Element
  ...
  //  Generate Tags List from Input item
      ...
      if(!options.readonly) {
          $("#add").click(function () {
              var value = $tag_input_elem.val().substr(1, $tag_input_elem.val().length).split(',');
              for (var i = 0; i < value.length; i++) {
                  var selected_tag = value[i];
  ...
  //  Methods
      ...
      function generate_tags_list(tags_list) {
        ...
        jQuery.each(tags_list, function(key, val) {
          if(val != "") {
            var remove_tag_link = (options.readonly) ? '' : '<a href="javascript:void(0)" class="'+options.className+'-remove '+options.className+'-remove-'+uuid+'" title="" rel="'+val+'">'+options.removeTagText+'</a>';
