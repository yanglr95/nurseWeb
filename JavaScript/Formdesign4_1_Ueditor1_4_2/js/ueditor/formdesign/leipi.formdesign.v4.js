...
UE.plugins['custom_field'] = function () {

    var me = this;
    var thePlugins = "custom_field";
    var fieldModel;
    me.commands[thePlugins] = {
        execCommand: function (command, field) {
            fieldModel = field
            var dialog = new UE.ui.Dialog({
                iframeUrl: 'CustomField.html',
                name: thePlugins,
                editor: this,
                title: '自定义字段',
                cssRules: "width:450px;height:400px;",
                field: field,
                buttons: [
                {
                    className: 'edui-okbutton',
    ...
    var popup = new baidu.editor.ui.Popup({
        ...
        className: 'edui-bubble',
        _edittext: function () {
            baidu.editor.plugins[thePlugins].editdom = popup.anchorEl.parentNode;
            me.execCommand(thePlugins, fieldModel);
            this.hide();
        },
        _delete: function () {
            if (window.confirm('确认删除该控件吗？')) {
    ...
    me.addListener('mouseover', function (t, evt) {
        evt = evt || window.event;
        var el = evt.target || evt.srcElement;
        var customfield = el.getAttribute('custom_field');
        if (customfield) {
            if (/div/ig.test(el.tagName) && customfield) {
                var html = popup.formatHtml(
                    '<nobr>自定义字段:' + customfield + ' <span onclick=$$._edittext() class="edui-clickable">编辑</span>&nbsp;&nbsp;<span onclick=$$._delete() class="edui-clickable">删除</span></nobr>');
                if (html) {
                    var elInput = el.getElementsByTagName("input");
                    var rEl = elInput.length > 0 ? elInput[0] : el;
                    popup.getDom('content').innerHTML = html;
